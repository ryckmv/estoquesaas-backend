...
export async function cadastrarEmpresaComAdmin(dados) {
    const { empresa, nome, email, senha, cnpj } = dados;
    if (!empresa?.trim() ||
        !nome?.trim() ||
        !email?.trim() ||
        !senha) {
        throw new AppError('Preencha empresa, nome, email e senha.', 400);
    }
    if (senha.length < 6) {
    ...
    try {
        ...
        if (erro.code === 'P2002') {
            throw new AppError('Já existe uma empresa cadastrada com esse CNPJ.', 400);
        }
        throw erro;
    }
